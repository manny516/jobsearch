...
export default function Jobitem({jobData} : any) {
  const jobItemsData = jobData.data.jobs;
  const jobCompany = jobItemsData;
  let duplicateCache : any = {};
  jobCompany.forEach((element : any, index : any)=>{
  ...
  return ( 
           ...
           jobItemsData.map((job: JobsCheck, index : number) => (
            <>
            {
              (duplicateCache[job.company.id] == false) ? 
            
                <a key={index} href={`/?pid=${job.company.id}`}>
                  <article  className="border-solid border-2 m-2 flex flex-row results-job-search bg-neutral-50  rounded-lg p-8">
                    ...
                    <article className="m-0 p-4 basis-9/12">
                          ...
                          <p className=" text-indigo-300"> {job.title}</p>
                          <blockquote className='flex text-slate-500'>
                            {(job.locationNames != null) ? 
                              <span className='text-xs '><FaMapMarker className='inline-block  mr-1'/>{job.locationNames}</span>  
                            : 
                              <span className='text-xs '><FaMapMarker className='inline-block  mr-1'/>Location Unavailable </span>  
                            }
                    ...
                    { duplicateCache[job.company.id]  = true}
                  </a> 
                  : ''
                  }
            </>
